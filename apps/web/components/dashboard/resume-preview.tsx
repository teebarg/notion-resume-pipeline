...
export function ResumePreview({ data, template }: ResumePreviewProps) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Import from Notion or load sample data to preview
        </p>
      </div>
    )
  }
  ...
  const templateStyles = {
    ...
    developer: {
      ...
      section: 'mb-5',
      sectionTitle: 'text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 mb-3 flex items-center gap-2 before:content-["//"]',
            ...
            {data.projects?.map((project, i: number) => (
              <div key={i}>
                <div className="flex items-center gap-2">
                  ...
                  {project.link && (
                    <ExternalLink className={cn('h-3 w-3', styles.accent)} />
                  )}
                </div>
